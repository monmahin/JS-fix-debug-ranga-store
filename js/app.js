...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <h6>Category: <span class="text-secondary">${product.category}</span></h6>
      <br>
      <h2>Price: $ <span class="text-danger">${product.price}</span></h2>
      <br>
      
      
      <h6>Rating: <span class="badge bg-secondary">${product.rating.rate}</span></h6>
      
      <h6>Review: <span class="badge bg-secondary">${product.rating.count}</span></h6>
      <br>
      <div class="d-flex justify-content-around">
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="btn btn-primary d-block">Add Cart</button>
      <button onclick="detailsBtn(${product.id})" class="btn btn-danger d-block">Details</button>
      </div>
      </div>
...
const getInputValue = (id) => {
  ...
  const fixConverted = parseFloat(converted.toFixed(2));
  return fixConverted;
};
